...
describe("slop rules", () => {
  effect(
    "unjustified-casts reports trust, not casts the checker can already confirm",
    () =>
      Effect.gen(function* () {
  ...
  effect(
    "unused-code separates never used, test-only, used and public",
    () =>
      Effect.gen(function* () {
  ...
  effect(
    "unused-optional-parameters needs every caller to skip it, through any receiver",
    () =>
      Effect.gen(function* () {
        const findings = yield* findingsOf(
  ...
  effect(
    "duplicated-functions preserves whitespace inside strings, templates and regexes",
    () =>
      Effect.gen(function* () {
        const findings = yield* findingsOf(
  ...
  effect(
    "duplicated-functions reports copies across files, not short or same-file ones",
    () =>
      Effect.gen(function* () {
        const body =
        ...
        const findings = yield* findingsOf(
          duplicatedFunctions({ within: "src/**", minimumLength: 40 }),
          {
            ...
            "src/c.ts":
              `export const different = (root: string, file: string): string => [file, root].map((part) => part.trim()).join("/")\n`,
          },
        )
        expect(findings).toEqual([
