...
describe("patterns", () => {
  ...
  effect("shape checks its fields against the node type already in the pipe", () =>
  ...
  effect("match tags each node with the first pattern that fits and narrows its captures", () =>
    withProject({ "src/shapes.ts": SOURCE }, (project) =>
      Effect.gen(function* () {
  ...
  effect("typedCaptures gives every captured node its type so a rule can branch on it", () =>
    withProject({ "src/shapes.ts": SOURCE }, (project) =>
      Effect.gen(function* () {
        ...
        expect(found.every(({ value }) => value.types.test !== undefined)).toBe(true)
      }),
    ),
  )
})
